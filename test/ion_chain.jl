...
@testset "Harmonic potential" begin
    ...
    function check_harmonic(coeffs, func)
    ...
    check_harmonic(IC.poly_function(Val(2))...)
    coeffs = zeros(MVector{2})
    function f(x)
        return x * evalpoly(x, Tuple(coeffs))
    end
    function ∇f(x)
        cs = Tuple(coeffs)
        cs2 = ntuple(i->i * cs[i], Val(2))
        return evalpoly(x, cs2)
    end
    check_harmonic(coeffs, IC.Function1D(f, ∇f))
    check_harmonic(coeffs, IC.Function1D(f))
end
